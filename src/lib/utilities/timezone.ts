...
export const getTimzoneValueFromLabel = (label: string) => {
	/*
...
export const transformGmtZoneName = (value: string): string =>
	value.replace(gmtZoneRegex, (_, extractedIdentifier: string) =>
		...
		extractedIdentifier.replace(
			/^GMT(?:([+-])(\d+))?$/,
			(__, sign: string, offset: string) => {
...
const compareGmtStrings = (a: string, b: string) =>
	parseInt(a.replace(gmtZoneRegex, '$2'), 10) -
	parseInt(b.replace(gmtZoneRegex, '$2'), 10);
...
export const momentToTimeInputValue = (
	time: Moment = moment(),
